...
Blockly.FieldVariable.dropdownChange = function(text) {
  ...
  function promptName(promptText, defaultText, isNew) {
    Blockly.hideChaff();
	
    var newVar = null;
	if (!isNew) {
		bootbox.prompt(promptText, function(result) {                
		  if (result === null) {                                             
		  } else {
		      result = result.replace(/[\s\xa0]+/g, ' ').replace(/^ | $/g, '');
		      if (result == Blockly.Msg.RENAME_VARIABLE ||
		          result == Blockly.Msg.NEW_VARIABLE) {
		        // Ok, not ALL names are legal...
		        result = null;
		      }
			  
			  if (result) {
				  Blockly.Variables.renameVariable(defaultText, result, thisInstance.sourceBlock_.workspace);
			  }
		  }
		});
	} else {
		...
		bootbox.prompt(promptText, function(result) {                
		  if (result === null) {                                             
			  ...
			  if (result) {
				  Blockly.Variables.renameVariable(newVar, result, thisInstance.sourceBlock_.workspace);
			  }
		  }
		});
	}
  ...
  if (text == Blockly.Msg.RENAME_VARIABLE) {
    ...
    text = promptName(Blockly.Msg.NEW_VARIABLE_TITLE, '', true);
