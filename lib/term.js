...
Term.init = function() {
	...
	Term.div.keydown(function(event) {
		...
		if (typeof Term.keyCodeMap[charCode] != 'undefined') {
			chrome.serial.send(Term.port.connId, Whitecat.str2ab(Term.keyCodeMap[charCode]), function(info) {
			});			
...
Term.ansiCodes = {
	...
	"[6n": function() {
		...
		chrome.serial.send(Term.port.connId, Whitecat.str2ab(String.fromCharCode(27) + "[" + (Term.y + 1) + ";" + (Term.x + 1)), function(info) {
		});			 		
	},	
}
