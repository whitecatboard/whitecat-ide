...
Blockly.Blocks['factory_base'] = {
	...
	spawnOutputShadow_: function(option) {
		...
		switch (option) {
			...
			case 'TOP':
				...
				this.connectOutputShadow_('PARENTTYPE');
				break;
			case 'BOTTOM':
	...
	updateShape_: function(option) {
		...
		var parentExists = this.getInput('PARENTTYPE');
		if (option == 'LEFT') {
			if (!outputExists) {
		...
		if (option == 'TOP' || option == 'BOTTOM' || option == 'BOTH') {
			if (!parentExists) {
				this.addTypeInput_('PARENTTYPE', 'parent type');
			}
		} else {
			this.removeInput('PARENTTYPE');
		}
	},
